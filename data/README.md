...
### **sample_signature_activity.txt**
  ...
  Please note that the signatures shown on the web (click [here]
  (http://adage.greenelab.com/#/signature/search?mlmodel=1) to see a demo) will
  be in alphabetical order. If you would like them to be ranked in another
  order, you may have to pad some signatures' name (such as changing `Sig9`
  into `Sig009`).
