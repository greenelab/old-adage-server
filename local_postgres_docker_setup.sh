...
# Run this script with 'sudo -u local_postgres_docker_setup.sh'

# This script defaults to postgres version 9.3.
# To use a different version set the POSTGRES_VERSION env var
# (i.e. 'sudo POSTGRES_VERSION=9.4 -u local_postgres_docker_setup.sh')
...
sed -i "s/#\(listen_addresses\)\( = 'localhost'\)/\1 = '*'/" /etc/postgresql/$POSTGRES_VERSION/main/postgresql.conf
