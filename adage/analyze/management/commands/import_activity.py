...
def valid_node_names(nodes, ml_model_name):
    ...
    for index, name in enumerate(nodes):
        if not name or name.isspace():
            logger.error("Line #1 column #%d: blank node name" % (index + 2))
            return False
        elif name in node_set:
            logger.error("Line #1 column #%d: %s is NOT unique" %
                         (index + 2, name))
            return False
        elif Node.objects.filter(name=name,
                                 mlmodel__title=ml_model_name).exists():
            logger.error("Line #1 column #%d: Node name already exists in "
                         "Node table: %s" % (index + 2, name))
            return False
        else:
            node_set.add(name)
...
def valid_data_line(line_num, data_line):
    ...
    if not data_source or data_source.isspace():
        logger.error("Line #%d: data_source is blank)" % line_num)
        return False

    if not Sample.objects.filter(ml_data_source=data_source).exists():
        logger.warn("Line #%d: data_source value not found in database: %s)" %
    ...
    for index, value in enumerate(values):
        try:
            ...
            logger.error(
                "Error on line #%d column #%d: %s is NOT float type" %
                (line_num, index + 2, value))
            return False
    return True
...
def valid_activity(file_handler, ml_model_name):
    ...
    If any error is detectd, it will generate an error message and return
    ...
    for line in file_handler:
        ...
        if line_num == 1:
            ...
            logger.warn("Error on line #%d: Number of fields is not %d" %
                        (line_num, col_num))
            return False
        elif not valid_data_line(line_num, fields):
            return False
